...
namespace RandomVideoPlayer
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string configFile = Path.Combine(configFolderPath, "config.json");
            if (!File.Exists(configFile))
            {
                ...
                SaveConfig(configFile, folderPath);
            }

            // Load the folder path from the config file
            string videoFolderPath = LoadConfig(configFile);
        ...
        static void SaveConfig(string configFile, string folderPath)
        {
            // Serialize the folder path to JSON and save it to the config file
            string json = JsonConvert.SerializeObject(folderPath, Formatting.Indented);
            File.WriteAllText(configFile, json);
        }

        static string LoadConfig(string configFile)
        {
            if (File.Exists(configFile))
            {
                // Read the JSON from the config file and deserialize it to get the folder path
